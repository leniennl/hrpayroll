...
# add gross. Calculate
merged_rate["Pay Elements Total"] = (
    merged_rate["Counter Hours"] * merged_rate["Multiplier"] * merged_rate["Trans Rate"]
)

# Calculate Pay Element Total
gross = merged_rate.groupby("Employee code")["Pay Elements Total"].sum().reset_index()
gross["Pay Elements Total"] = gross["Pay Elements Total"].round(2)

# Create an Excel writer object
with pd.ExcelWriter(
    filefolders + "/Kronos Timesheets Pay Element Check.xlsx"
) as writer:
